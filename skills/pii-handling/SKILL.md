...
> **Human required:** Apply any warehouse-native masking policies and IAM/role grants generated by this skill. Claude can produce the DDL and policy YAML, but cannot execute `ALTER TABLE ... SET MASKING POLICY` statements in Snowflake or configure BigQuery column-level IAM — a human with the appropriate warehouse admin role must apply these.
